...
class FacebookData(object):
    ...
    def search_for_pages(self, string_to_search_for):
        ...
        pages = graph.search(
            ...
            center='0,0',
            distance=5000,
            fields='name, emails, location, about, website, link')
    ...
    def iterate_through_pages(self, data):
        for place in data:
            try:
                if place['location']['country'] == 'United States':
                    print '{}, {}, {}, {}, {}'.format(
                        place['name'] if 'name' in place else '',
                        ','.join(place['emails'] if 'emails' in place else ''),
                        place['link'] if 'link' in place else '',
                        place['website'] if 'website' in place else '',
                        place['location']['city'])
            except:
                pass
...
if __name__ == '__main__':
    ...
    states_list1 = ['Texas', 'California']
    for state in states_list:
        pages = fb1.search_for_pages(search_term + state)
        fb1.print_page_data(pages, 200)
